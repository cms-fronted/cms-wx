<template>
  <div class="flex-column">
    <div class="flex-row" style="padding: 10px 20px 0 20px;">中餐</div>
    <!-- 订餐数量 -->
    <div
      class="flex-row"
    ...
    <div
      class="flex-row"
      style="justify-content: space-between;align-items:centerfont-size: 15px;color: #EE0A24;padding: 10px 20px 0 20px;"
      ref="mealType"
    >
    ...
    <div class="dateMenu">
      <van-tabs color="#1989fa">
        <van-tab :title="$moment(date).format('MM-DD')"></van-tab>
      </van-tabs>
    </div>
    <!-- 菜品及数量选择 -->
...
import { bindCanteen, getChooseDinner } from "@/api/user.js";
import { getFoodList, changeFoods, getDinnerInfo } from "@/api/selfDish.js";
    ...
    async submitOrder() {
      ...
      this.$route.params.orderDetail.foods.forEach((items, index) => {
        add.forEach((item, key) => {
          if (
            items.food_id == item.add_foods.food_id &&
            items.count <= item.add_foods.count
          ) {
            add.splice(key, 1);
          }
        });
