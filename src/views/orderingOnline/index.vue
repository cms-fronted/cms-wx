<template>
	...
	<div>
		...
		<table width="100%" ref="tableForm">
			<tr>
				<th>日期</th>
				<th v-for="(item,index) in mealList" :data-d_id="item.id" :data-type="item.type" :data-type_number="item.type_number"
				 :data-time="item.limit_time" :data-count="item.ordered_count" :key="index">{{item.name}}</th>
			</tr>
			<tr>
				<td>全选</td>
        ...
        <td>全晚</td>-->
			...
			<tr v-for="(item,index) in tableData" :key="index">
				<td>{{item.date}}</td>
				<td v-for="(order, orderIndex) in item.orderOfMeal" @touchstart="(e)=>gotouchstart(e,item)" @touchmove="gotouchmove"
				 @touchend="(e)=>gotouchend(e,item)" :key="orderIndex">
					<p v-for="canteen in order.canteens" :key="canteen.canteen_id">{{canteen.canteen}}*{{canteen.count}}</p>
				</td>
			</tr>
		</table>
		<van-popup v-model="timeShow" position="bottom" :style="{ height: '40%' }">
			<van-datetime-picker v-model="currentDate" type="year-month" :min-date="minDate" @cancel="timeShow = false" @confirm="timeConf" />
		</van-popup>
		<van-dialog v-model="editShow" title="修改数量" :before-close="(action,done)=>confirmEdit(action,done)"
		 :show-cancel-button="true">
			<van-stepper style="padding:10px 0" input-width="40px" button-size="32px" v-model="nowCount" min="1" :max="maxCount" />
		</van-dialog>
		<van-dialog v-model="addShow" title="订餐数量" :before-close="(action,done)=>confirmAdd(action,done)" :show-cancel-button="true">
			<van-stepper style="padding:10px 0" input-width="40px" button-size="32px" v-model="addCount" min="1" :max="maxCount" />
		</van-dialog>
	</div>
</template>
