<template>
	...
	<div id="totalorder">
		...
		<van-popup class="van-popup" v-model="detail" closeable round :style="{ height: '50%' }">
			<van-list v-model="detailLoading" :finished="detailFinished" finished-text="没有更多了" @load="onLoadDetail">
		...
		<van-popup class="van-popup" v-model="personDetail" closeable round :style="{ height: '50%' }">
			<van-list v-model="personDetailLoading" :finished="personDetail1Finished" @load="onLoadPersonDetail">
				<p class="
		 popTitle">订餐就餐人员明细</p>
				<div class="mealAmount">
					<table border="0" cellspacing="0" cellpadding="0">
		...
		<van-popup class="van-popup" v-model="personDetail1" closeable round :style="{ height: '50%' }">
			<van-list v-model="personDetail1Loading" :finished="personDetail1Finished" @load="onLoadPersonDetail1">
				<p class="
		 popTitle">未订餐就餐人员明细</p>
				<div class="mealAmount">
					<table border="0" cellspacing="0" cellpadding="0">
						<tr style="line-height:30px;">
							<td style="width: 30%;">名称</td>
		...
		<van-popup class="van-popup" v-model="personDetail2" closeable round :style="{ height: '50%' }">
			<van-list v-model="personDetail2Loading" :finished="personDetail2Finished" @load="onLoadPersonDetail2">
				<p class="
		 popTitle">订餐未就餐人员明细</p>
	...
	export default {
		...
		data() {
			return {
				...
				personDetail1Loading: false, //未订餐就餐
				personDetail1Finished: false,
				personDetail2Loading: false, //订餐未就餐
				personDetail2Finished: false,
			}
		},
		methods: {
			async onLoadDetail() {
				if (this.paging.last_page != null) {
					if (this.paging.current_page == this.paging.last_page) {
						...
						if (result.errorCode == 0 && result.data.haveFoods == 1 && result.data.statistic) {
							// 设置分页数据
							this.paging.total = result.data.statistic.total;
							this.paging.last_page = result.data.statistic.last_page;
							//初始化页面数据
							this.menuDetail.list = this.menuDetail.list.concat(result.data.statistic.data);
						}
					};
				...
				if (this.paging.last_page != null) {
					if (this.paging.current_page == this.paging.last_page) {
						...
						if (result.errorCode == 0) {
							...
							this.personList.list = this.personList.list.concat(result.data.data);
						}
					};
					this.detailLoading = false;
				};
