<template>
  <div id="app">
    <!-- 饭堂选择下拉箭头 -->
    <van-icon
      style="position: fixed;right: 10px;top: 40px;z-index: 10;"
      size="40px"
      name="arrow-down"
      @click="showCanteen"
    />
    ...
    <div class="shadow" style="margin-bottom: 10px;" v-bind:hidden="show">
      <van-radio-group
        ...
        style="width: 100%;display: flex;flex-wrap: wrap;padding-top: 35px"
      >
        <van-radio
...
import { getUserToken, canChooseCant } from "./api/user.js";
...
export default {
  ...
  methods: {
    ...
    async chooseCanteen(e) {
      await this.$store.commit("user/setCanteen", e);
