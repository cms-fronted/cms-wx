			...
			<div class="shadow" style="background-color: #FFFFFF;margin-bottom: 10px">
				...
				<div v-bind:hidden="show">
					<van-radio-group v-model="radio" @change="chooseCanteen" style="width: 100%;display: flex;flex-wrap: wrap;">
						<van-radio class="flex-row flex-center" :name="item.id" icon-size="12px" v-for="(item, index) in canteenList"
						 :key="index" style="width: 33%; margin: 10px 0">{{ item.name }}</van-radio>
					</van-radio-group>
				</div>
			</div>
		</van-sticky>
...
<script>
	...
	export default {
		...
		methods: {
			...
			bindToWhat(phone, canteen,canteen_id) {
				if (phone == 2) {
					this.$router.push('entry');
				} else if (canteen == 2) {
					this.$router.push('setting');
				} else {
					this.$store.commit("user/setCanteen", canteen_id);
		...
		async mounted() {
			...
			if (!token && !code && !state) {
				...
				//非第一次进入 非重定向进入
				this.bindToWhat(phone, canteen_selected,canteen_id)
